...
def maze2d_set_terminals(env):              ### Finds out when the trajectory reaches the goal - same goal of (1,1) for all the Maze2D 
    env = load_environment(env) if type(env) == str else env
    goal = np.array(env._target)
    print("goal",goal)
    threshold = 0.5
    
    #### FIND THE GOAL FUCNTION USING TIMEOUTS ####    Here if we constrain the positions we wont ever real the GOAL

    #def _fn(dataset): 
        #return dataset
  
    def _fn(dataset):
        print("dataset start", dataset)
       #"dataset['observations']",dataset['observations'])
        xy = dataset['observations'][:,:2]   ## array([[0.4      , 0.8      ]
        #print("dataset['observations'][:,:2]",dataset['observations'][:,:2])
        print("size of xy",xy.shape)     # (1000000, 2)
        distances = np.linalg.norm(xy - goal, axis=-1)
        at_goal = distances < threshold                      ## When r u less than  0.5 tob the Goal?
        timeouts = np.zeros_like(dataset['timeouts'])             # array([False, False, False, ..., False, False, False])
        ## Find the actual timeouts from the dataset!!

        ## timeout at time t iff
        ##      at goal at time t and
        ##      not at goal at time t + 1
        timeouts[:-1] = at_goal[:-1] * ~at_goal[1:] ## Find the moment the agent leave the Goal  array([False, False, False, ..., False, False, False])
        
        # If at_goal = [False, True, True, False], then:
        # at_goal[:-1] = [False, True, True]
        # at_goal[1:] = [True, True, False]
        # ~at_goal[1:] = [False, False, True]
        # at_goal[:-1] * ~at_goal[1:] = [False, False, True]
        
        
        # at_goal[:-1] * ~at_goal[1:] = [False, False, True]
        timeout_steps = np.where(timeouts)[0]     ## array([   267,   1467,   1856, ..., 998638, 998915, 999499])
        # np.where to find the indices where timeouts is 1:
        path_lengths = timeout_steps[1:] - timeout_steps[:-1]   ## array([1200,  389,  274, ..., 1376,  277,  584])
        ## how long it took from the start to get to the goal? What about the 128 NUMBER for each batch?
 
        # [ utils/preprocessing ] Segmented maze2d-umaze-v1 | 1565 paths | min length: 96 | max length: 3992 # 
        print(
        ...
        csv_file = 'outputWithGoalsLDSPostTimeOutss.csv'
