...
MAZE_BOUNDS = {
    ...
    'maze2d-medium-v1': (0, 8, 0, 8),     ## Maze Widths
...
class MazeRenderer:
    ...
    def renders(self, observations, conditions=None, title=None):
        ...
        plt.imshow(self._background*0 ,                          ### * by 0 to remove barriers
            extent=self._extent, cmap=plt.cm.binary, vmin=0, vmax=1)     ## axis just put in here OK!
        ...
        plt.scatter(observations[:,3], observations[:,2], c="black", zorder=1) # actual Co-ordinates ## zorder=20, c=colors ## Sampling Observations ...        
        plt.scatter(observations[:,1], observations[:,0], c=colors, zorder=1) # actual Co-ordinates ## zorder=20, c=colors ## Sampling Observations ... 
        #print("observations[:,1]",observations[:,1])         
        #print("observations[:,0]",observations[:,0]) 
        plt.axis('on')    
        plt.title(title)
    ...
    def composite(self, savepath, paths, ncol=5, **kwargs):
        ...
        imageio.imsave(savepath, images)
        print(f'Saved {len(paths)} samples to: {savepath}')    ### Note the Saving of the diagrams
...
class Maze2dRenderer(MazeRenderer):
    ...
    def __init__(self, env, observation_dim=None):
        ...
        self.action_dim = np.prod(self.env.action_space.shape)
        self.goal = None
        self._background = self.env.maze_arr == 10       ## Maze co-ords!?  ## Maze comes straight from the environment - just to stick on the images
        print("self._background",self._background)
        self._remove_margins = False 
        self._extent = (0, 1, 1, 0)
 
    def renders(self, observations, conditions=None, **kwargs):
        bounds = MAZE_BOUNDS[self.env_name]             ## Maze co-ords!?
        ...
        if len(bounds) == 2:
            ...
            #observations[:, 2] /= iscale     # observations[:, 0] /= iscale
            #observations[:, 3] /= jscale     # observations[:, 1] /= iscale
        else: 
